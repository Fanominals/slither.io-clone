...
export class Renderer {
    ...
    private draw3DHex(ctx: CanvasRenderingContext2D, points: {x:number, y:number}[], centerX: number, centerY: number, background = false) {
    ...
    drawGrid(): void {
        ...
        const grad = this.ctx.createRadialGradient(
            this.canvas.width / 2, this.canvas.height / 2, this.canvas.width * 0.15,
    ...
    private drawSnakeName(snake: ClientSnake): void {
        ...
        const text = snake.nickname; // Use nickname directly instead of getDisplayName()
